...
public class EmployeeService {
	...
	public void sortEmpsBySalary(boolean ascending) {
		  //Create a comparator object that compares employees by salary
		  Comparator<Employee> salaryComparator = new Comparator<Employee>() {
		    @Override
		    public int compare(Employee e1, Employee e2) {
		      //If ascending is true, compare salaries normally
		      if (ascending) {
		        return Double.compare(e1.getSalary(), e2.getSalary());
		      }
		      //If ascending is false, compare salaries in reverse order
		      else {
		        return Double.compare(e2.getSalary(), e1.getSalary());
		      }
		    }
		  };
		  //Sort the employee set using the comparator
		  TreeSet<Employee> sortedSet = new TreeSet<>(salaryComparator);
		  sortedSet.addAll(empset);
		  //Display the sorted employees
		  for (Employee emp : sortedSet) {
		    System.out.println(emp);
		  }
		}
